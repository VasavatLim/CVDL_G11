...
def evaluate(writer, step, dataloader, model, loss_fn):
    ...
    for batch in dataloader:
        input, output = batch["img"].to(DEVICE), batch["msk"].to(DEVICE)
...
def main():
    ...
    transform_msk = transforms.Compose(
        [
            ...
            transforms.Lambda(
                lambda x: torch.nn.functional.one_hot(x, 3).squeeze(0).permute(2, 0, 1)
            ),
    ...
    def transform(samples):
        ...
        samples["msk"] = [transform_msk(img) for img in samples["msk"]]
        return samples

    # load dataset & apply transform
    ds = load_dataset("cvdl/oxford-pets")
    ds = ds.select_columns(["img", "msk"])
